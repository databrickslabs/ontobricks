...
class TestSettingsRoutes:
    ...
    def test_settings_save(self, client):
        response = client.post('/settings/save', json={
...
class TestOntologyRoutes:
    ...
    def test_generate_owl(self, client):
        payload = {
            'name': 'Test', 'base_uri': 'http://test.org#',
            'classes': [{'name': 'X', 'label': 'X'}], 'properties': []
        }
...
class TestMappingRoutes:
    ...
    def test_save_mapping(self, client):
        config = {
            'entities': [{'ontology_class': 'A', 'id_column': 'id', 'sql_query': 'SELECT *'}],
            'relationships': [],
        }
    ...
    def test_delete_entity_mapping(self, client):
        client.post('/mapping/entity/add', json={
            'ontology_class': 'http://test/Del',
            'ontology_class_label': 'Del',
            'sql_query': 'SELECT *',
            'id_column': 'id',
        })
        response = client.post('/mapping/entity/delete', json={'ontology_class': 'http://test/Del'})
        assert response.status_code == 200
    ...
    def test_generate_r2rml(self, client):
        client.post('/ontology/save', json={
            'name': 'T', 'base_uri': 'http://test.org#',
...
class TestDomainRoutes:
    ...
    def test_import_domain(self, client):
        domain_data = {
            ...
            'versions': {
                '1': {
                    'ontology': {'name': 'O', 'base_uri': 'http://t#', 'classes': [], 'properties': [],
...
class TestAPIv1Routes:
    ...
    def test_validate_query(self, client):
        response = client.post('/api/v1/query/validate',
