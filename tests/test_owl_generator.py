...
def _make_generator(classes=None, properties=None, constraints=None, swrl_rules=None, axioms=None, expressions=None):
...
class TestPropertyGeneration:
    ...
    def test_datatype_property(self):
        props = [{
            'name': 'age', 'label': 'Age',
            'type': 'DatatypeProperty', 'domain': 'Customer', 'range': 'xsd:integer'
        }]
        gen = _make_generator(properties=props)
        owl = gen.generate()
        assert 'DatatypeProperty' in owl
...
class TestConstraintGeneration:
    ...
    def test_min_cardinality(self):
        constraints = [{
            'type': 'minCardinality', 'property': 'hasOrder',
            'className': 'Customer', 'cardinalityValue': 1
        }]
        gen = _make_generator(constraints=constraints)
        owl = gen.generate()
...
class TestExpressionGeneration:
    ...
    def test_intersection_of(self):
        expressions = [{'type': 'intersectionOf', 'subject': 'WorkingParent', 'objects': ['Parent', 'Employee']}]
        gen = _make_generator(expressions=expressions)
        owl = gen.generate()
...
class TestRoundtrip:
    ...
    def test_roundtrip_swrl(self):
        rules = [{
            'name': 'TestRule', 'description': 'Test',
            'antecedent': 'A(?x)', 'consequent': 'B(?x)'
        }]
        gen = _make_generator(swrl_rules=rules)
        owl = gen.generate()
