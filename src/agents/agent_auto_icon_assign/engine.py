...
from agents.agent_auto_icon_assign.tools import (
    ToolContext, TOOL_DEFINITIONS, TOOL_HANDLERS,
)
...
def run_agent(
    ...
    logger.info(
        ...
        endpoint_name, len(entity_names),
    )
    ...
    for iteration in range(MAX_ITERATIONS):
        ...
        try:
            llm_response = call_serving_endpoint(
                ...
                tools=send_tools, temperature=0.3,
            ...
            if exc.response is not None and status in (400, 422) and tools_supported:
                logger.warning("Icon Agent: endpoint rejected tools — falling back to direct mode")
                ...
                logger.error("Icon Agent: LLM error at iteration %d: %s", iteration + 1, exc)
                return result
            ...
            result.error = f"LLM request failed: {exc}"
            logger.error("Icon Agent: request error at iteration %d: %s", iteration + 1, exc)
            return result
        ...
        if tool_calls:
            ...
            for tc in tool_calls:
                ...
                result.steps.append(AgentStep(
                    ...
                    content=(tool_result[:300] + "…") if len(tool_result) > 300 else tool_result,
                    tool_name=tool_name,
                    duration_ms=tool_ms,
                ))
            ...
            if ctx.icon_results:
                ...
                logger.info(
                    ...
                    result.iterations, len(result.icons),
                )
                return result
        else:
            ...
            if parsed:
                logger.info("Icon Agent: parsed %d icons from text fallback", len(parsed))
    ...
    result.error = f"Agent reached maximum iterations ({MAX_ITERATIONS}) without assigning icons"
