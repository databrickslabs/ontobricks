...
class RDFSParser:
    ...
    def _parse_content(self, content: str):
        ...
        try:
            ...
            raise ValidationError("Could not parse RDFS content. Supported formats: Turtle, RDF/XML, N3, N-Triples, JSON-LD")
    ...
    def get_classes(self) -> List[Dict[str, str]]:
        ...
        for cls in self.graph.subjects(RDF.type, RDFS.Class):
            ...
            if uri.startswith('http://www.w3.org/1999/02/22-rdf-syntax-ns#') or \
    ...
    def _extract_class_info(self, cls, uri: str) -> Dict[str, str]:
        ...
        for parent_cls in self.graph.objects(cls, RDFS.subClassOf):
            ...
            if not parent_uri.startswith('_:') and \
               not parent_uri.endswith('Resource') and \
               not parent_uri.endswith('Thing'):
        ...
        try:
            ...
            for prop in self.graph.subjects(RDF.type, OWL.ObjectProperty):
                ...
                if uri.startswith('_:') or uri in seen_uris:
                    continue
                seen_uris.add(uri)
                properties.append(self._extract_property_info(prop, uri, 'ObjectProperty'))
            ...
            for prop in self.graph.subjects(RDF.type, OWL.DatatypeProperty):
                ...
                if uri.startswith('_:') or uri in seen_uris:
                    continue
                seen_uris.add(uri)
                properties.append(self._extract_property_info(prop, uri, 'DatatypeProperty'))
        except ImportError:
            pass
    ...
    def _extract_property_info(self, prop, uri: str, prop_type: str) -> Dict[str, str]:
        ...
        xsd_types = {
            'string', 'integer', 'int', 'float', 'double', 'decimal', 'boolean', 'bool',
            'date', 'dateTime', 'time', 'duration', 'gYear', 'gMonth', 'gDay',
            'hexBinary', 'base64Binary', 'anyURI', 'QName', 'NOTATION',
            'normalizedString', 'token', 'language', 'NMTOKEN', 'Name', 'NCName',
            'nonPositiveInteger', 'negativeInteger', 'long', 'short', 'byte',
            'nonNegativeInteger', 'unsignedLong', 'unsignedInt', 'unsignedShort', 'unsignedByte',
            'positiveInteger', 'Literal', 'langString'
        }
