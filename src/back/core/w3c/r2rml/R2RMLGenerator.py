...
class R2RMLGenerator:
    ...
    def generate_mapping(self, mapping_config: Dict[str, Any],
                         ontology_config: Dict[str, Any] = None) -> str:
        ...
        all_dsm = mapping_config.get('entities', mapping_config.get('data_source_mappings', []))
        ...
        all_rm = mapping_config.get('relationships', mapping_config.get('relationship_mappings', []))
        ...
        for idx, rel_mapping in enumerate(relationship_mappings):
            self._add_relationship_mapping(g, rel_mapping, entity_lookup, property_lookup, idx)
        
        return g.serialize(format='turtle')
    
    def _build_property_lookup(self, ontology_config: Dict[str, Any] = None) -> Dict[str, Dict[str, str]]:
    ...
    def _add_entity_mapping(self, g: Graph, config: Dict[str, Any], idx: int,
        ...
        if sql_query:
            ...
            g.add((logical_table, self.rr.tableName, Literal(f"{catalog}.{schema}.{table}")))
        ...
        class_name = self._extract_local_name(class_uri) if class_uri else (class_label or table)
        class_name = self._sanitize_name(class_name)
        
        g.add((subject_map, self.rr.template, 
        ...
        if class_uri:
            if class_uri.startswith('http://') or class_uri.startswith('https://'):
                ...
                g.add((subject_map, self.rr['class'], URIRef(f"{self.base_uri}{class_uri}")))
        ...
        if attribute_mappings:
            for attr_name, column_name in attribute_mappings.items():
                ...
                if ont_uri and ont_uri.startswith(self.base_uri):
                    ...
                    attr_uri = URIRef(f"{self.base_uri}{self._sanitize_name(attr_name)}")
    ...
    def _add_relationship_mapping(self, g: Graph, config: Dict[str, Any], 
        ...
        source_column = config.get('source_column') or config.get('source_id_column', '')
        target_column = config.get('target_column') or config.get('target_id_column', '')
        sql_query = config.get('sql_query', '').strip()
        ...
        if not source_class_label or not target_class_label or not source_class_uri or not target_class_uri:
            prop_info = property_lookup.get(property_uri) or property_lookup.get(property_label) or {}
        ...
        if direction == 'reverse':
            ...
            source_class_label, target_class_label = target_class_label, source_class_label
        ...
        source_entity = self._lookup_entity(entity_lookup, source_table, source_class_label, source_class_uri)
        target_entity = self._lookup_entity(entity_lookup, target_table, target_class_label, target_class_uri)
        ...
        # Create unique identifier for this TriplesMap
        prop_name = self._sanitize_name(property_label or self._extract_local_name(property_uri))
        ...
        g.add((triples_map, RDFS.comment, 
               Literal(f"Relationship mapping: {source_label} --{property_label}--> {target_label}")))
        ...
        if sql_query:
            ...
            if source_sql:
                ...
                if catalog and schema and source_table:
                    g.add((logical_table, self.rr.tableName, 
        ...
        source_class_name = self._resolve_class_name(
            source_class, source_class_uri, source_class_label,
        ...
        if property_uri.startswith('http://') or property_uri.startswith('https://'):
            if property_uri.startswith(self.base_uri):
                ...
                g.add((pom, self.rr.predicate, URIRef(f"{self.base_uri}{self._sanitize_name(local)}")))
        else:
        ...
        target_class_name = self._resolve_class_name(
            target_class, target_class_uri, target_class_label,
            target_entity, target_table
        )
        
        # ALWAYS use taxonomy base URI for object template
        g.add((obj_map, self.rr.template, 
    ...
    def _lookup_entity(self, entity_lookup: Dict, table: str, class_label: str, class_uri: str) -> Dict:
        ...
        if class_uri:
            ...
            if not result:
                local_name = self._extract_local_name(class_uri)
                if local_name:
                    result = entity_lookup.get(local_name) or entity_lookup.get(local_name.lower())
        # Try class label
        if not result and class_label:
            result = entity_lookup.get(class_label) or entity_lookup.get(class_label.lower())
        # Try table name
        if not result and table:
            result = entity_lookup.get(table)
        return result or {}
    
    def _resolve_class_name(self, class_from_entity: str, class_uri: str, class_label: str,
                            entity: Dict, table: str) -> str:
        ...
        logger.warning("Could not resolve entity class name. class_uri='%s', class_label='%s', table='%s'",
                       class_uri, class_label, table)
