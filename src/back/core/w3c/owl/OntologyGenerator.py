...
class OntologyGenerator:
    ...
    def _add_cardinality_restriction_uri(self, restriction_type: str, prop_uri: URIRef, 
        ...
        if not class_uri or not prop_uri or value is None:
            logger.debug("Missing required values: class_uri=%s, prop_uri=%s, value=%s", class_uri, prop_uri, value)
            return
        ...
        logger.debug("Added cardinality restriction: %s subClassOf %s", class_uri, restriction)
    ...
    def _add_values_restriction_uri(self, restriction_type: str, prop_uri: URIRef, 
        ...
        self.graph.add((class_uri, RDFS.subClassOf, restriction))
        logger.debug("Added values restriction: %s subClassOf %s", class_uri, restriction)
    ...
    def _add_value_check_constraint(self, constraint: Dict):
        ...
        logger.debug("Added value check constraint: %s.%s %s '%s'", class_local, attribute_name, check_type, check_value)
    ...
    def _add_class(self, cls: Dict):
        ...
        logger.debug("Processing class: %s, parent: %s", class_name, cls.get('parent', 'NONE'))
    ...
    def _add_axiom(self, axiom: Dict):
        ...
        if axiom_type == 'equivalentClass' and subject_uri and objects:
            ...
            if obj_uris:
                ...
                logger.debug("Added disjointUnion: %s disjointUnionOf %s", subject, objects)
        ...
        # ── Class expressions (wrapped via owl:equivalentClass) ──
            ...
            if obj_uris:
                ...
                logger.debug("Added unionOf: %s equivalentClass unionOf(%s)", subject, objects)
            ...
            if obj_uris:
                ...
                self.graph.add((subject_uri, OWL.equivalentClass, anon))
                logger.debug("Added intersectionOf: %s equivalentClass intersectionOf(%s)", subject, objects)
            ...
            if obj_uri:
                ...
                self.graph.add((subject_uri, OWL.equivalentClass, anon))
                logger.debug("Added complementOf: %s equivalentClass complementOf(%s)", subject, objects[0])
        
        elif axiom_type == 'oneOf' and subject_uri:
            individuals_raw = axiom.get('individuals', '')
            if isinstance(individuals_raw, str):
                individuals = [i.strip() for i in individuals_raw.split(',') if i.strip()]
            else:
