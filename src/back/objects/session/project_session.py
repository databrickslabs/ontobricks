"""
Project session service — unified session management for OntoBricks.

Runtime payload is a nested dict with **project** (info, current_version,
project_folder, is_active_version, UC **metadata** tables, triplestore), **ontology**
...
``project.is_active_version``; generated OWL, R2RML, and SQL; and
...
def sanitize_project_folder(name: str) -> str:
    """Turn a project name into a safe folder name (lowercase, underscores)."""
...
def get_empty_project() -> Dict[str, Any]:
    """Return an empty project data structure."""
    return {
        "project": {
            "info": {
                "name": "NewProject",
...
class ProjectSession:
    """
    Unified project session manager.
    
    Usage:
        project = ProjectSession(session_mgr)
        
        # Get values
        name = project.info.name
        classes = project.ontology.classes
        
        # Set values
        project.info.name = "My Project"
        project.ontology.classes.append(new_class)
        
        # Save changes
        project.save()
    """
    
    SESSION_KEY = 'project_data'
    ...
    def _load(self) -> Dict[str, Any]:
        """Load project data from session, migrating legacy keys if needed."""
        # Try to get unified project_data first
    ...
    def _ensure_structure(self, data: Dict) -> Dict:
        """Ensure all required keys exist in project data and migrate legacy structures."""
        empty = get_empty_project()
        
        # Migrate from old flat structure to new /project structure
        if 'project' not in data:
            data['project'] = empty['project'].copy()
            if 'info' in data:
                data['project']['info'] = data.pop('info')
            if 'current_version' in data:
                data['project']['current_version'] = data.pop('current_version')
        
        # Always migrate is_active_version from root to project (even if project exists)
        if 'is_active_version' in data:
            data['project']['is_active_version'] = data.pop('is_active_version')

        # --- Migrate uc_location → project.project_folder -----------------
        old_uc = data.get('project', {}).pop('uc_location', None) or data.pop('uc_location', None)
        if old_uc and old_uc.get('project_folder'):
            data['project'].setdefault('project_folder', old_uc['project_folder'])
        ...
        if old_db:
            data['settings']['databricks'] = {**empty['settings']['databricks'], **old_db}
        ...
        if old_prefs:
            ...
            reg = data['settings'].setdefault('registry', dict(empty['settings']['registry']))
        ...
        # Ensure project sub-keys exist
        for subkey in empty['project']:
            if subkey not in data['project']:
                data['project'][subkey] = empty['project'][subkey]
            elif isinstance(empty['project'][subkey], dict):
                for subsubkey in empty['project'][subkey]:
                    if subsubkey not in data['project'][subkey]:
                        data['project'][subkey][subsubkey] = empty['project'][subkey][subsubkey]
        
        # Add missing top-level keys (excluding 'project' and 'settings' already handled)
        for key in empty:
        ...
        # ----------------------------------------------------------
        empty_ts = empty['project']['triplestore']
        ts = data['project'].setdefault('triplestore', {})
        for k in empty_ts:
            ts.setdefault(k, (empty_ts[k].copy() if isinstance(empty_ts[k], dict) else empty_ts[k]))

        info = data['project'].get('info', {})

        # Migrate project.delta -> project.triplestore.delta
        if 'delta' in data['project'] and data['project']['delta']:
            ts['delta'].update(data['project'].pop('delta'))

        # Drop legacy lakebase data (backend removed)
        if 'lakebase' in data['project']:
            data['project'].pop('lakebase')
        ...
        # Migrate project.info.triplestore_stats -> project.triplestore.stats
        if 'triplestore_stats' in info:
            ts['stats'] = info.pop('triplestore_stats')

        # Migrate legacy project.info.triplestore_table -> project.triplestore.delta
        ...
        if 'mapping' in data:
            ...
            data['assignment'] = {
                'entities': old_m.get('data_source_mappings', old_m.get('entities', [])),
                'relationships': old_m.get('relationship_mappings', old_m.get('relationships', [])),
        ...
        if 'design_layout' in data:
            ...
            if 'current_view' not in dl:
                dl['current_view'] = 'default' if 'default' in dl['views'] else (list(dl['views'].keys())[0] if dl['views'] else None)
        ...
        # Migrate root-level UC table metadata -> project.metadata
        if 'metadata' in data:
            legacy_m = data.pop('metadata', None)
            if isinstance(legacy_m, dict):
                data['project']['metadata'] = legacy_m
    ...
    def _migrate_legacy(self) -> Dict:
        ...
        # Migrate project_info / project_name into project.info
        project_info = self._session_mgr.get('project_info', {})
        if project_info:
            data['project']['info'].update(project_info)
        project_name = self._session_mgr.get('project_name')
        if project_name:
            data['project']['info']['name'] = project_name
        ...
        if ontology_config:
            ...
            data['ontology']['constraints'] = self._session_mgr.get('property_constraints', [])
        ...
        if mapping_config:
            data['assignment'] = {
                'entities': mapping_config.get('entities', mapping_config.get('data_source_mappings', [])),
                'relationships': mapping_config.get('relationships', mapping_config.get('relationship_mappings', [])),
                'r2rml_output': mapping_config.get('r2rml_output', '') or self._session_mgr.get('r2rml_output', '')
            }
        ...
        if design_layout:
            ...
            if 'default' not in design_layout['views'] or not design_layout['views'].get('default'):
                if old_entities or old_relationships or old_inheritances:
            ...
            if 'current_view' not in design_layout:
                design_layout['current_view'] = 'default' if 'default' in design_layout['views'] else (list(design_layout['views'].keys())[0] if design_layout['views'] else None)
    ...
    def save(self):
        ...
        Stamps ``project.last_update`` when the ontology or mapping data
        ...
        if self._config_snapshot() != self._initial_config_snapshot:
            self._data['project']['last_update'] = datetime.now(timezone.utc).isoformat()

        ont_snap, asgn_snap = self._split_snapshots()
        if ont_snap != self._initial_ontology_snapshot:
            self._data['project']['ontology_changed'] = True
        if asgn_snap != self._initial_assignment_snapshot:
            self._data['project']['assignment_changed'] = True
        ...
        # Legacy top-level keys (migrated under project / ontology)
    ...
    def get_session_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'project_name': self.info.get('name', 'NewProject'),
        }
    ...
    def reset_ontology(self) -> None:
        ...
        self.ontology.update({
            'name': '', 'base_uri': '', 'description': '',
            'classes': [], 'properties': [],
            'constraints': [], 'swrl_rules': [], 'axioms': [], 'expressions': [],
        })
    ...
    def clear_uc_metadata(self):
        ...
        """
        self._data['project']['project_folder'] = ''
        self._data['project']['is_active_version'] = True
    ...
    def _clear_legacy_keys(self):
        ...
        legacy_keys = [
            'project_info', 'project_name', 'project_config',
            'ontology_config', 'property_constraints', 'swrl_rules', 'owl_axioms',
            'mapping_config', 'r2rml_output',
            'design_layout',
            'databricks_host', 'databricks_token', 'warehouse_id', 'catalog', 'schema',
            'default_emoji', 'generated_owl', 'generated_sql', 'query_result'
        ]
    ...
    def project(self) -> Dict[str, Any]:
        """Get project metadata (info, current_version, project_folder)."""
        return self._data['project']
    
    @property
    def info(self) -> Dict[str, Any]:
        """Get project info."""
        return self._data['project']['info']
    
    @property
    def current_version(self) -> str:
        """Get current version."""
        return self._data['project'].get('current_version', '1')
    
    @current_version.setter
    def current_version(self, value: str):
        """Set current version."""
        self._data['project']['current_version'] = value
    
    @property
    def project_folder(self) -> str:
        """The folder name under which this project is saved in the registry."""
        return self._data['project'].get('project_folder', '')

    @project_folder.setter
    def project_folder(self, value: str):
        self._data['project']['project_folder'] = value

    @property
    def uc_project_folder(self) -> str:
        """Sanitized project folder name for registry storage."""
        folder = self.project_folder
        if folder:
            return folder
        return sanitize_project_folder(self.info.get('name', 'untitled_project'))
    ...
    @property
    def uc_project_path(self) -> str:
        """Full /Volumes/.../projects/<folder> path, or '' if registry not set.
    ...
    def deduplicate_shacl_shapes(self) -> None:
        ...
        if dropped:
            ...
            logger.info("Deduplicated SHACL shapes: removed %d duplicates (%d → %d)", dropped, len(existing), len(result))
    ...
    def catalog_metadata(self) -> Dict[str, Any]:
        """Unity Catalog table/column metadata (stored under ``project``)."""
        return self._data['project'].setdefault('metadata', {})
    ...
    def triplestore(self) -> Dict[str, Any]:
        ...
        return self._data['project'].get('triplestore', {})
    ...
    def ensure_generated_content(self) -> Dict[str, bool]:
        ...
        if self.get_entity_mappings() and not self.get_r2rml():
            try:
                ...
                r2rml_content = generator.generate_mapping(self.assignment, self.ontology)
    ...
    def export_for_save(self) -> Dict[str, Any]:
        ...
        """
        # Get current version from project node
        version = self._data['project'].get('current_version', '1')
        
        # Export info from project.info (without version - version is at versions level)
        info_export = {
            'name': self._data['project']['info'].get('name', 'NewProject'),
            'description': self._data['project']['info'].get('description', ''),
            'author': self._data['project']['info'].get('author', ''),
            'llm_endpoint': self._data['project']['info'].get('llm_endpoint', ''),
            'mcp_enabled': self._data['project']['info'].get('mcp_enabled', False),
            'last_update': self._data['project'].get('last_update', ''),
            'last_build': self._data['project'].get('last_build', ''),
        }
    ...
    def import_from_file(self, data: Dict[str, Any], version: str = None):
        """Import project data from loaded file.
        ...
        # Import info into project.info
        if 'info' in data:
            info = data['info']
            self._data['project']['info']['name'] = info.get('name', 'NewProject')
            self._data['project']['info']['description'] = info.get('description', '')
            self._data['project']['info']['author'] = info.get('author', '')
            self._data['project']['info']['llm_endpoint'] = info.get('llm_endpoint', '')
            self._data['project']['info']['mcp_enabled'] = info.get('mcp_enabled', False)
            self._data['project']['last_update'] = info.get('last_update', '')
            self._data['project']['last_build'] = info.get('last_build', '')
            ts = self._data['project'].setdefault('triplestore', get_empty_project()['project']['triplestore'].copy())
            ts.pop('backend', None)
            # Migrate legacy triplestore_table into triplestore.delta
            ts_full = info.get('triplestore_table', '')
            if ts_full:
                parts = ts_full.split('.')
                d = ts.setdefault('delta', get_empty_project()['project']['triplestore']['delta'].copy())
                if len(parts) >= 3:
        ...
        if 'versions' in data:
            ...
            if version_keys:
                ...
                self._data['project']['current_version'] = selected_version
                ...
                if 'assignment' in version_data or 'mapping' in version_data:
                    ...
                    ad = version_data.get('assignment') or version_data.get('mapping', {})
                    self._data['assignment'] = {
                        'entities': ad.get('entities', ad.get('data_source_mappings', [])),
                        'relationships': ad.get('relationships', ad.get('relationship_mappings', [])),
                ...
                if 'metadata' in version_data:
                    self._data['project']['metadata'] = version_data['metadata']
                else:
                    self._data['project']['metadata'] = {}
        else:
            # Legacy flat format
            # Handle version from info (old format had version in info)
            if 'info' in data and 'version' in data['info']:
                self._data['project']['current_version'] = data['info']['version']
            ...
            if 'metadata' in data:
                self._data['project']['metadata'] = data['metadata']
            else:
                self._data['project']['metadata'] = {}
        
        # Import delta into project.triplestore
        empty_ts = get_empty_project()['project']['triplestore']
        ts = self._data['project'].setdefault('triplestore', empty_ts.copy())
        if 'delta' in data:
...
def get_project(session_mgr) -> ProjectSession:
    """Get or create a ProjectSession for this request.

    The instance is cached on ``request.state`` so that multiple
    callers within the same request (e.g. middleware + route handler)
    share one ``ProjectSession`` and avoid repeated
    ``_ensure_structure()`` work.
    """
    request = getattr(session_mgr, 'request', None)
    if request is not None:
        cached = getattr(request.state, '_project_session', None)
        if cached is not None:
            return cached
    ps = ProjectSession(session_mgr)
    if request is not None:
        request.state._project_session = ps
    return ps
