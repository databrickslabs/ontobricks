...
class DomainSession:
    ...
    def _ensure_structure(self, data: Dict) -> Dict:
        ...
        if 'domain' in data and 'domain_folder' not in data['domain'] and 'project_folder' in data['domain']:
        ...
        old_uc = data.get('domain', {}).pop('uc_location', None) or data.pop('uc_location', None)
            ...
            if subkey not in data['domain']:
                ...
                for subsubkey in empty['domain'][subkey]:
                    if subsubkey not in data['domain'][subkey]:
                        data['domain'][subkey][subsubkey] = empty['domain'][subkey][subsubkey]
    ...
    def _clear_legacy_keys(self):
        ...
        legacy_keys = [
            'domain_info', 'project_name', 'project_config',
    ...
    def import_from_file(self, data: Dict[str, Any], version: str = None):
        ...
        if 'info' in data:
            ...
            ts = self._data['domain'].setdefault('triplestore', get_empty_domain()['domain']['triplestore'].copy())
            ts.pop('backend', None)
            # Migrate legacy triplestore_table into triplestore.delta
            ts_full = info.get('triplestore_table', '')
            if ts_full:
                parts = ts_full.split('.')
                d = ts.setdefault('delta', get_empty_domain()['domain']['triplestore']['delta'].copy())
                if len(parts) >= 3:
