...
from back.objects.domain._metadata_tasks import run_metadata_load_task, run_metadata_update_task
...
class Domain:
    ...
    def import_domain(self, domain_data: Dict[str, Any], 
    ...
    @staticmethod
    def list_domain_versions_result(svc: RegistryService, domain_name: str) -> Dict[str, Any]:
    ...
    def load_domain_from_uc(
        ...
        try:
            ...
            if not version:
                ...
                if active_ver:
                    version = active_ver
                    logger.info("No version specified for '%s'; resolved to active v%s", domain_name, version)
                else:
    ...
    def create_new_domain_version(self, svc: RegistryService) -> Dict[str, Any]:
        ...
        try:
            ...
            if copied:
                logger.info("Copied %d document(s) from V%s to V%s", copied, current_version, new_version)
            if doc_errors:
    ...
    def get_version_status(self, refresh: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            reg = self._s.registry
            domain_folder = self._s.domain_folder
            has_registry = bool(reg.get('catalog') and reg.get('volume') and domain_folder)
