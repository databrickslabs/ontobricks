...
async def list_registry_domains(
    ...
    return DomainsResponse(
        success=True,
        domains=[DomainInfo(name=p['name'], description=p['description']) for p in items],
    )
...
async def get_domain_design_status(
    ...
    meta = domain._data.get('domain', domain._data.get('project', {})).get('metadata', {})
    ...
    logger.info(
        ...
        dname, ontology_valid, len(tables),
...
async def get_domain_ontology(
    ...
    logger.info(
        ...
        domain.domain_folder or '(session)', len(classes), len(properties),
    )
...
async def get_domain_r2rml(
    ...
    logger.info(
        ...
        domain.domain_folder or '(session)', len(entities), len(relationships),
    )
...
async def get_domain_sparksql(
    ...
    try:
        ...
        logger.info(
            ...
            domain.domain_folder or '(session)', len(sql_content),
        )
