/**
 * OntoBricks - domain-owl-content.js
...
window.loadOwlContent = async function() {
    ...
    try {
        // Use the export-owl endpoint which generates OWL from the domain ontology
...
document.getElementById('downloadOwlBtn')?.addEventListener('click', async function() {
    ...
    const filename = domainName.replace(/\s+/g, '_') + '.ttl';
