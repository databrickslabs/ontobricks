/**
 * OntoBricks - project-metadata.js
...
async function loadMetadataStatus() {
    ...
    statusText.innerHTML = '<span class="spinner-border spinner-border-sm me-2" role="status"></span>Loading metadata…';
    ...
    try {
        ...
        if (data.success && data.has_metadata) {
            ...
            // Extract catalog.schema from first table's full_name for display
            let locationDisplay = 'Multiple locations';
            if (tableCount > 0 && data.metadata.tables[0].full_name) {
                const parts = data.metadata.tables[0].full_name.split('.');
                if (parts.length >= 2) {
                    locationDisplay = `${parts[0]}.${parts[1]}`;
                }
            }
            ...
            tableSelections = {};
            statusDiv.className = 'alert alert-secondary mb-4';
            statusText.innerHTML = '<i class="bi bi-info-circle"></i> No metadata loaded';
            previewDiv.style.display = 'none';
        }
    } catch (error) {
        console.error('Error loading metadata status:', error);
        statusDiv.className = 'alert alert-warning mb-4';
        statusText.innerHTML = '<i class="bi bi-exclamation-triangle"></i> Error loading metadata status';
    }
}
...
async function loadMetadataFromUC() {
    ...
    try {
        ...
        const response = await fetch('/project/metadata/list-tables', {
            method: 'POST',
...
async function importSelectedTables() {
    ...
    loadBtn.disabled = true;
    progressDiv.style.display = 'block';
    statusSpan.textContent = `Loading metadata for ${selectedTables.length} table(s)...`;
    
    try {
        // Start async task
        const response = await fetch('/project/metadata/initialize-async', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                catalog, 
        ...
        showNotification('Metadata loading started...', 'info');
            ...
            if (task.status === 'completed') {
                ...
                if (task.result && task.result.metadata) {
                    try {
                        ...
                        const saveResp = await fetch('/project/metadata/save', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                ...
                showNotification(task.message || 'Metadata operation completed!', 'success');
...
function displayMetadataPreview(metadata) {
    ...
    tbody.innerHTML = html || '<tr><td colspan="4" class="text-center text-muted">No tables found</td></tr>';
...
async function removeSelectedTables() {
    if (!metadataCache) {
        showNotification('No metadata loaded', 'warning');
        return;
    }
    ...
    const confirmed = await showConfirmDialog({
        title: isRemovingAll ? 'Clear All Metadata' : 'Remove Tables',
        message: isRemovingAll 
            ? 'Are you sure you want to remove all tables? This will clear the metadata.'
            : `Are you sure you want to remove ${toRemoveCount} table${toRemoveCount !== 1 ? 's' : ''} from the metadata?`,
    ...
    try {
        ...
        if (isRemovingAll) {
            const response = await fetch('/project/metadata/clear', {
                method: 'POST',
                credentials: 'same-origin'
            });
            ...
            if (data.success) {
                showNotification('Metadata cleared', 'success');
                metadataCache = null;
                tableSelections = {};
                await loadMetadataStatus();
            } else {
            ...
            const response = await fetch('/project/metadata/save', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    tables: tablesToKeep
...
async function saveMetadataChanges(silent = false) {
    if (!metadataCache) {
        if (!silent) showNotification('No metadata to save', 'warning');
        return;
    }
    ...
    try {
        ...
        if (data.success) {
            if (!silent) showNotification(`Saved metadata for ${allTables.length} tables`, 'success');
        } else {
            showNotification('Error: ' + data.message, 'error');
        }
    } catch (error) {
...
function showTableDetails(tableIndex) {
    ...
    // Use full_name - it's required in the new structure
    const fullName = table.full_name || table.name;
    document.getElementById('tableDetailsName').textContent = fullName;
...
async function clearMetadata() {
    const confirmed = await showConfirmDialog({
        ...
        message: 'Are you sure you want to clear the loaded metadata?',
        confirmText: 'Clear',
        confirmClass: 'btn-danger',
        icon: 'trash'
    });
...
async function updateMetadataFromUC() {
    ...
    if (!metadataCache || !metadataCache.tables || metadataCache.tables.length === 0) {
        showNotification('No metadata loaded to update', 'warning');
        return;
    }
    ...
    try {
        // Start async task
        const response = await fetch('/project/metadata/update-async', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
