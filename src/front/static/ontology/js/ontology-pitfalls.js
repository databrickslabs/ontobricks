...
window.PitfallsModule = (function () {
    ...
    const STATIC_TAXONOMY = [
        { category: 'Logical Issues',              pitfall_id: 'P1.1', title: 'Parent disjoint with children',                      description: 'An entity is declared disjoint from one of its own subclasses. This is a contradiction: no individual can simultaneously belong to both an entity and its subclass if they are disjoint, making the subclass unsatisfiable (always empty).' },
        { category: 'Logical Issues',              pitfall_id: 'P1.2', title: 'Entity as subclass of both parent and grandparent',   description: 'An entity is explicitly declared as a direct subclass of both a parent entity and one of that parent\'s ancestors. The declaration to the ancestor is redundant because subclass transitivity already implies it, and it can mislead reasoners.' },
        { category: 'Logical Issues',              pitfall_id: 'P1.3', title: 'Logical inconsistencies',                             description: 'Axioms in the ontology produce unsatisfiable entities — e.g., an entity is simultaneously defined as a subclass of two disjoint entities, or a restriction forces contradictory types. An OWL reasoner would flag these entities as equivalent to owl:Nothing.' },
        { category: 'Structural Issues',           pitfall_id: 'P2.1', title: 'Not connected hierarchies',                           description: 'The ontology contains isolated entity trees with no common root other than owl:Thing. Disconnected hierarchies often indicate that separate sub-ontologies were merged without alignment, or that top-level grouping concepts are missing.' },
        { category: 'Structural Issues',           pitfall_id: 'P2.2', title: 'Single subclass parent',                              description: 'An entity has exactly one direct subclass. This is often a sign of unnecessary intermediate nodes: if a parent has only one child, the hierarchy could usually be flattened without loss of semantics, unless the parent is used in axioms independently.' },
        { category: 'Structural Issues',           pitfall_id: 'P2.3', title: 'Superfluous disjointness',                            description: 'Two entities are declared disjoint when one is already a subclass of the other, or when disjointness is already implied by the hierarchy. The explicit disjointness assertion is redundant and may create unintended logical side-effects.' },
        ...
        { category: 'Redundancy / Naming Issues',  pitfall_id: 'P3.3', title: 'Possible hierarchy among entities',                   description: 'Two or more entity names are so similar that a subclass relationship may exist but is not declared. This check flags candidate pairs for human review to determine whether a missing rdfs:subClassOf should be added.' },
        ...
        { category: 'Semantic Issues',             pitfall_id: 'P4.5', title: 'Unconnected entities (no usage)',                     description: 'One or more entities are never used as domain, range, or type in any property assertion or restriction. Such orphan entities add vocabulary without contributing to the ontology\'s modelling or reasoning.' },
        { category: 'Semantic Issues',             pitfall_id: 'P4.6', title: 'Semantically equivalent labels',                      description: 'Two or more entities or properties carry labels that are semantically similar or synonymous (detected via sentence-embedding cosine similarity). These may be duplicates or candidates for merging, equivalence declarations, or at least a clarifying note.' },
        { category: 'Semantic Issues',             pitfall_id: 'P4.7', title: 'Missing labels',                                      description: 'One or more entities or properties have no rdfs:label. Labels are essential for human-readable documentation, UI display, and tool interoperability. Every named entity in a published ontology should carry at least one label.' },
    ];
    ...
    function _renderResults(result) {
        ...
        detailEl.textContent =
            `${meta.classes || 0} entities · ${meta.object_properties || 0} object properties · ` +
