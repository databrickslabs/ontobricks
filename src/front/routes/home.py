...
async def home_page(request: Request):
    """Home page."""
    return templates.TemplateResponse(request, "home.html", {"app_version": APP_VERSION})
...
async def settings_page(request: Request, session_mgr: SessionManager = Depends(get_session_manager)):
    """Settings page."""
    user_role = getattr(request.state, "user_role", "admin")
    return templates.TemplateResponse(request, "settings.html", {"user_role": user_role})
...
async def access_denied_page(request: Request):
    ...
    return templates.TemplateResponse(request, "access_denied.html", {"user_email": email})
