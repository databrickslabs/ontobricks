...
`master` branch folder `/docs` (legacy branch deploy). The `docs/` tree is
...
- Repository documentation stays in `docs/` (Markdown / Sphinx). It is no
...
note in `docs/` or README pointing at the live Pages URL once `/site` is
live.
